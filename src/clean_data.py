...
def data_prep(file_pattern, out_path, label_col):
    timestr = time.strftime('%Y%m%d')
    files = glob.glob(file_pattern)
    df_path = max(files, key=os.path.getmtime)
    unwanted_cols = os.getenv('UNWANTED_COLS').split(',')
    clean_obj = CleanData(df_path, out_path, unwanted_cols, num_how='mean', cat_how='mode')
    df = clean_obj.clean_csv_data()
    encode_obj = Encoders(df, label_col)
    df = encode_obj.one_hot()
    out_file = f"{out_path}/{timestr}_{label_col}_encoded.csv"
...
def main():
    ...
    parser.add_argument("file_pattern", type=str,
                        help="Pattern name of the dataset to clean and prepare.")
    parser.add_argument("out_path", type=str,
                        help="Path to where to place the cleaned and encoded dataset.")
    parser.add_argument("label_col", type=str,
                        help="Label column.")
    args = parser.parse_args()
    data_prep(args.file_pattern, args.out_path, args.label_col)
